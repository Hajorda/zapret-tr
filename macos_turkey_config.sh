...
# Bu işlem sayesinde Apple'ın kendi internet kuralları (com.apple) silinmez ve sertifika/HTTPS hatası yaşanmaz!
awk '
/^anchor "com.apple\/\*"/ {
...
# PF Kurallarını test et ve yükle
sudo pfctl -ef /tmp/zapret_pf_combined.conf &>/dev/null

# Kapanışta kuralları temizle (Orijinal macOS kurallarına geri dön)
